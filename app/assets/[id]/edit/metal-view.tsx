...
// Server view for a precious-metal holding: live valuation KPIs + buy/sell
// entry. Shares the holdings math and the buy/sell component with crypto.
export default async function MetalView({
  asset,
  locale,
}: {
  ...
  const [prices, usdGel] = await Promise.all([
    asset.symbol ? fetchMetalPrices([asset.symbol]) : Promise.resolve<Record<string, number>>({}),
    fetchUsdGel(),
  ]);
  ...
  // CryptoTrades reads crypto_quantity / crypto_unit_price labels.
  labels.crypto_quantity = labels.metal_holdings;
  labels.crypto_unit_price = labels.metal_unit_price;
  ...
  return (
    <main>
      ...
      <section className="kpi-grid" style={{ marginTop: 8 }}>
        <div className="kpi">
          <div className="kpi__label">{t(locale, "metal_holdings")}</div>
          <div className="kpi__value">{qty}</div>
